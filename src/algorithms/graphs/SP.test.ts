...
describe("SP", () => {
  ...
  describe("relax", () => {
    it("takes the edge when it shortens the known distance", () => {
      const sp = new SP(new WDigraph(3, [new DiWEdge(0, 1, 4)]), 0);
      const shortcut = new DiWEdge(0, 1, 1);

      expect(sp.relax(shortcut)).toBe(true);
      expect(sp.distTo[1]).toBe(1);
      expect(sp.edgeTo[1]).toBe(shortcut);
    });

    it("leaves everything alone when the edge is no improvement", () => {
      const sp = new SP(new WDigraph(3, [new DiWEdge(0, 1, 4)]), 0);
      const before = sp.edgeTo[1];

      expect(sp.relax(new DiWEdge(0, 1, 4))).toBe(false);
      expect(sp.relax(new DiWEdge(0, 1, 10))).toBe(false);
      expect(sp.distTo[1]).toBe(4);
      expect(sp.edgeTo[1]).toBe(before);
    });

    it("cannot relax an edge leaving an unreached vertex", () => {
      const sp = new SP(new WDigraph(3, []), 0);

      expect(sp.relax(new DiWEdge(1, 2, 1))).toBe(false);
      expect(sp.distTo[2]).toBe(Infinity);
      expect(sp.edgeTo[2]).toBeUndefined();
    });
  });
