...
function buildFullTree(): BST<number> {
...
describe("BinarySearchTree", () => {
  ...
  describe("put|get", () => {
    ...
    it("deletes the key if exists and the value passed is null", () => {
      const firstKey = keys[0];
      tree.put(firstKey, null);
      expect(tree.contains(firstKey)).toBe(false);
    });
  ...
  describe("delete", () => {
    ...
    it("removes a leaf (0 children)", () => {
      const full = buildFullTree();
      full.delete(4);
      expect(full.contains(4)).toBe(false);
      expect(full.keys()).toEqual([1, 2, 3, 5, 6, 7, 8, 9]);
    });
